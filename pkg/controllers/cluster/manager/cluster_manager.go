...
func (m *ClusterManager) getClusterStatus(ctx context.Context) (*innodb.ClusterStatus, error) {
	...
	if localMSHErr != nil {
		glog.V(6).Infof("[getClusterStatus] localMySh.GetClusterStatus err: %v", localMSHErr)
		var err error
	...
	glog.V(6).Infof("[getClusterStatus] ok: clusterStatus: %v", clusterStatus)
...
func (m *ClusterManager) Sync(ctx context.Context) bool {
	...
	if err != nil {
		...
		if m.Instance.Ordinal == 0 {
			...
			glog.V(6).Info("[Sync] myshErr: %v", myshErr)
			return false
		}
	}
