...
func (r *runner) StartGroupReplication(ctx context.Context) error {
	python := fmt.Sprintf("session.query('start group_replication')")
	_, err := r.run(ctx, python)
	return err
}

func (r *runner) ShutdownInstance(ctx context.Context) error {
	python := fmt.Sprintf("session.query('SHUTDOWN')")
	_, err := r.run(ctx, python)
	return err
}
